...
def prophet_model(df):
    df_p = df.rename(columns={"Time of Call": "ds", "call_count": "y"})[["ds", "y"]]
...
for granularity in ["hourly", "daily", "weekly", "monthly"]:
    print(f"\n🔍 Processing A7 {granularity}")

    df = pd.read_csv(f"A7/A7_{granularity}.csv")
...
results_df = pd.DataFrame(overall_results).sort_values("overall_rmse")

# Save final result
results_df.to_csv("A7/A7_best_model.csv", index=False)
