...
print("--- Loading M40 (Monthly) MovingAverage Model ---")

MODEL_NAME = "A40\\M40_monthly.joblib"
DATA_NAME = "A40\\A40_monthly_calls.csv"
...
# --------------------------------------------------
def predict():
    """Predict NEXT MONTH call count for API A40 using Moving Average."""
    ...
    try:
        ...
        prediction = int(
            max(0, round(np.mean(ARTIFACT["last_values"])))
        )
